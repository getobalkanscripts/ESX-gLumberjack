...
RegisterNetEvent('esx-lumberjack:BuyAxe', function()
    ...
    if not axe then
        ...
        exports['mythic_notify']:DoHudText('success', 'Kupio si sjekiru za drva')
        --xPlayer.showNotification(Config.Alerts["axe_bought"], true, false, 140)
    elseif axe then
        exports['mythic_notify']:DoHudText('success', 'Vec imas sjekiru')
...
RegisterServerEvent('esx-lumberjack:lumberprocessed', function()
    ...
    if lumber.count < 1 then 
        exports['mythic_notify']:DoHudText('error', 'Nemas dovoljno Drveta')
    ...
    if lumber.count >= amount then 
        ...
        exports['mythic_notify']:DoHudText('success', 'Uspjesno si zamjenio drva za daske')
        ...
        Wait(750)
        xPlayer.addInventoryItem('wood_plank', TradeAmount)
    else 
...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    print("--------------------------------------------------------------------------")
    print("^4                  ESX DRVOSJECA QTARGET BY AMMCOOO                     |")
